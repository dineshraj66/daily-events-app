...
export default function App() {
  ...
  const [statsCatFilter, setStatsCatFilter] = useState(null); // for stats drill-down
  ...
  useEffect(() => {
    if (!userId) return;
    const eventsUnsub = onSnapshot(collection(db, "users", userId, "events"), (snap) => {
      setEvents(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
      setLoading(false);
    });
    const catUnsub = onSnapshot(doc(db, "users", userId, "settings", "categories"), (snap) => {
  ...
  useEffect(() => {
    ...
    sleepToCreate.forEach(({ sleepStart, sleepEnd }) => {
      ...
      setDoc(doc(db, "users", userId, "events", id), {
